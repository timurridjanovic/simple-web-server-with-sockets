...
def start_server():
    ...
    while True:
        ...
        (body, response) = app(request_dict)
...
def app(environ):
    if environ["request"] == "GET":
        return handle_get_request(environ)
    elif environ["request"] == "POST":
        return handle_post_request(environ)

def handle_get_request(environ):
    path_vars = [e for e in environ["path"].split("/") if e]

    if environ["path"] in router:
        return (router[environ["path"]].get(), "text/html")

    elif path_vars[0] == "css" and len(path_vars) > 1:
        return (serve_css(environ["path"]), "text/css")

    elif path_vars[0] == "js" and len(path_vars) > 1:
        return (serve_javascript(environ["path"]), "application/javascript")
...
def handle_post_request(environ):
    if environ["path"] in router:
        return (router[environ["path"]].post(environ["fields"]), "text/html")
    else:
        return (["404 Error"], "text/html")
...
class Index(object):
    @classmethod
    def get(self):
        return serve_page("index", {"name": "Timur", "phone": "123-4567"})
